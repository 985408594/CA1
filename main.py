...
def run():
    image = cv2.imread('D:/FYP CA1 code/20180531_135129.jpg')  # a black objects on white image is better
    image = cv2.resize(image,(800,600))
    # gray = cv2.cvtColor(image.copy(), cv2.COLOR_BGR2GRAY)
    # ret, thresh = cv2.threshold(gray, 127, 255, cv2.THRESH_BINARY)
    thresh = cv2.Canny(image,50,100)
    #my_matrix = np.loadtxt(open("D:\\1.csv", "rb"), delimiter=",", skiprows=0)
    #np.savetxt('1.csv', my_matrix, delimiter = ',')
    np.savetxt('D:/new.csv', thresh, delimiter=',')
